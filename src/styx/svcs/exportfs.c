...
exportproc(void *a)
{
	...
	for(;;){
        ...
        if (exdebug) print("exportproc: waking slave\n");
		Wakeup(&exq.rwait);
        if (exdebug) print("exportproc: sent wakeup\n");
	}
...
exshutdown(Export *fs)
{
	...
	/* work not started */
    if (exdebug) print("exportfs: shutting down export\n");
    if (exdebug) print("exportfs: removing unstarted work\n");
	lock(&exq.l);
    ...
    if (exdebug) print("exportfs: killing work in progress\n");

	lock(&fs->l);
	while((q = fs->work) != nil){
		fs->work = q->next;
		lock(&q->l);
        if (exdebug) print("exportfs: issuing cancelation order\n");
		q->shut = 1;
