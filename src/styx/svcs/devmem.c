...
memread(Chan *c, void *va, long count, vlong offset)
{
	...
	case Qgc:
        ...
        /* FIX: Its trying to print garbage collector stats that are only available in the external
         *      global variables gcnruns, gcsweeps etc.  These might be available within the Lua VM
         *      but its going to take a while to integrate them.  For the time being we'll just define
         *      them all to be zero in libnode.c and fix when we fully integrate the VM.
         */
