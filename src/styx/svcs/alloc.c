...
struct
{
	...
	3,
	{
		{ "main",  0, 	32*1024*1024, MQUANTA,  512*1024, 0, 31*1024*1024 },
		{ "heap",  1, 	32*1024*1024, MQUANTA,  512*1024, 0, 31*1024*1024 },
		{ "image", 2,   32*1024*1024+256, MQUANTA, 4*1024*1024, 1, 31*1024*1024 },
	}
};
...
/* poolimmutable hides *v from the garbage collector */
/* poolmutable allows the garbage collector to reap *v if possible */

/* their interactions with the DIS vm are very-specific.  until we 
 * determine what allocator to use for the kernel code and which to
 * use for the lua code, this is just commented out
 */
 
/* FIX: ADAPT to lua if necessary.
void
...
/* NOTE: The dis GC uses 'ptr' (seemingly) to refer to a current item or boundary in
 * the 'heap' memory pool.  I'm pretty sure it interacts solely with dis since it's
 * it's garbage collector is the only thing that requires this type of synchronization.
 * It's use here seems to be some type of interaction with the free regions in the GC
 * and in the heapmem pool.  We probably don't need it, but to keep it consistent 
 * we initiaze it the same way as the GC by setting it to pool->chains.
 */
void
poolfree(Pool *p, void *v)
{
    ...
    trace(TRACE_DEBUG, "kernel/poolfree: freeing %p in pool %p",v,p);
	D2B(b, v);
    trace(TRACE_DEBUG, "kernel/poolfree: %p is in block %p",v,b);
	if(p->monitor)
	...
	if(c->magic == MAGIC_F) {	/* Join forward */
        trace(TRACE_DEBUG, "kernel/poolfree: joining block %p with next free block %p",b,c); 
		if(c == ptr) {
			ptr = b;
            trace(TRACE_DEBUG, "kernel/poolfree: sync ptr forward to b: %p",b);
        }
		pooldel(p, c);
        trace(TRACE_DEBUG, "kernel/poolfree: zeroing magic c=%p, c->magic=%lx, &(c->magic)=%p",c,c->magic, &(c->magic));
		c->magic = 0;
		b->size += c->size;
		B2T(b)->hdr = b;
	}

    trace(TRACE_DEBUG, "converting block to prev tail: blk: %p, sizeof(Btail) %ld, (uchar*)b - sizeof(Btail): %p",b, sizeof(Btail), (uchar*)b - sizeof(Btail));
	c = B2PT(b)->hdr;
	if(c->magic == MAGIC_F) {	/* Join backward */
        trace(TRACE_DEBUG, "kernel/poolfree: joining block %p with prev free block %p",b,c); 
		if(b == ptr) {
			ptr = c;
            trace(TRACE_DEBUG, "kernel/poolfree: sync ptr backward to c: %p",c);
        }
		pooldel(p, c);
        trace(TRACE_DEBUG, "kernel/poolfree: zeroing magic b=%p, b->magic=%lx, &(b->magic)=%p",b,b->magic, &(b->magic));
		b->magic = 0;
		c->size += b->size;
		b = c;
		B2T(b)->hdr = b;
...
// alternate bindings for shared libs
void*
imalloc(size_t size)
{
	void *v; 
...
void*
icalloc(size_t n, size_t szelem)
{
    return malloc(n*szelem);
}
