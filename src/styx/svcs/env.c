...
closeegrp(Egrp *e)
{
	...
	if(e == nil || decref(&e->r) != 0)
		return;
    trace(TRACE_DEBUG, "kernel/closeegrp: releasing entries");
    
	for (el = e->entries; el != nil; el = nl) {
        trace(TRACE_DEBUG, "kernel/closeegrp: releasing variable '%s' @ %p with value '%.*s'",el->var, el->var,el->val?el->len:0,el->val?el->val:"nil");
		free(el->var);
		if (el->val) {
			free(el->val);
        }
