...
compiler instead of the Dis virtual machine.  It also uses the libuv
...
channels.  In fact, in Node9 each user session is allocated its own 
OS thread. Node9 threads can communicate via application channels and OS
pipes.

Node9 boots into a shell which supports both immediate mode Lua expression
evaluation as well as loading Lua applications and library modules.  

Node9 applications are Lua scripts which can be simple standalone command 
files as well as "application directories" containing all application code, 
support libraries, resources and configuration settings.  This allows 
applications to be easily installed and removed.

Like Plan9 and Inferno, Node9 distributed networks communicate via 9p resource
sharing, optionally using SSL/TLS connections.  (See the Inferno installation
manual for details).
...
compiler and cloud-optimized kernel.
...
patched to support 64-bit operation.  LuaJIT apps are still limited to 2GB.
...
to create a portable, reactor-based kernel we used the hardware abstraction
...
The legacy kernel is small, uses a round-robin scheduler and includes numerous
...
straight jacket.  It has a very readable syntax similar to Python.  The semantics
are very similar to the Scheme dialect of LISP, but using hashes rather than lists
as the basic data structure.

It's rare to find such a combination of power and simplicity.
...
host operating system.   Node9 uses libuv to portably support both POSIX and
Windows threads.
...
with each other via event channels (more on that later).  An application can
have multiple cooperating tasks.

Virtual Procs: A virtual proc is the kernel's shadow image of the Lua task while
processing system requests.  It contains the process ID, process structures and
system request buffers for the task.  

When Lua issues system requests, the kernel dispatches the request into an 
...
events with the Windows asynchronous I/O completion model.  This allows the same
code base to support high-performance on POSIX and Windows hosts.  It evolved 
originally from the free libev system.

The libuv event loop in node9 controls the synchronization of events, timers,
and the dispatching of I/O handlers.  Each kernel proc has it's own event loop.

Each iteration of the libuv event loop looks like this:
...
system call response queue and transfers control to each Lua task that's 
ready to run.  After this it runs any other Lua tasks that are ready 
...
the process starts again.  A slight variation on this model might run the 
...
a background monitor in the event loop.  It's an somewhat arcane 
...
of the calling task.  Each task (here called c_proc), has a sys request
buffer which conforms to the selected call.

The interface between the Lua system call and the C kernel call is handled by
the LuaJIT Foreign Function Interface (FFI).
...
New applications are normally started by /appl/sh.lua and here are a few
details of how this works.  The new application is forked from the current
task using the sys.spawn function and specifying command line arguments,
...
the case within Node9, though it's complicated by the fact that the 
...
(2) If an application error has occurred, but it's not a fatal program abort
...
Node9 contains embedded copies of it's dependent libraries "libuv" and
...
the source distribution) is set by the following rules:
...
sys.stream:
  reads or writes from src to dest until end of stream
...
scheduler.   These are:
  ...
  - devprog: allows fine-grain interaction with processes
...
is the conversion of all I/O drivers to libuv instead of the Inferno portability
layer.  This should simplify portability and increase performance significantly.
